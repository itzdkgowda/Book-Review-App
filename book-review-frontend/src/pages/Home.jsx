    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-950 text-white">
      <section className="text-center py-20 px-6">
        ...
        <div className="flex justify-center gap-4">
          <Link to="/add-book" className="bg-emerald-600 hover:bg-emerald-700 px-5 py-2 rounded text-white font-medium shadow">➕ Add Book</Link>
          <Link to="/add-review" className="bg-indigo-600 hover:bg-indigo-700 px-5 py-2 rounded text-white font-medium shadow">✍️ Add Review</Link>
        </div>
      </section>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 max-w-6xl mx-auto">
          {featured.length ? featured.map(book => (
            <div key={book._id} className="bg-white/10 p-5 rounded-xl shadow-md hover:scale-105 transition">
              ...
              <p className="text-slate-300">by {book.author}</p>
              {book.genre && (
                <span className="inline-block mt-2 px-3 py-1 text-xs bg-indigo-700 rounded-full">
                  {book.genre}
                </span>
              )}
            </div>
          )) : (
            <p className="text-slate-400 col-span-full text-center">No featured books yet.</p>
          )}
        </div>
      </section>
    </div>
