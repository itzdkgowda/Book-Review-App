      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <input
            ...
            className="flex-1 p-3 rounded bg-white/10 placeholder-gray-400"
          />
          <select value={genre} onChange={e => setGenre(e.target.value)} className="p-3 bg-white/10 rounded">
            {genres.map(g => <option key={g}>{g}</option>)}
          </select>
        </div>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          {filtered.length ? filtered.map(book => (
            <div key={book._id} className="bg-white/10 p-4 rounded-lg shadow">
              <h3 className="font-bold text-lg">{book.title}</h3>
              <p className="text-indigo-300 text-sm">by {book.author}</p>
              <Link to={`/books/${book._id}`} className="text-indigo-400 underline mt-2 inline-block">Details</Link>
            </div>
          )) : (
            <p className="text-slate-400 col-span-full">No books found.</p>
          )}
        </div>
      </div>
    </div>
